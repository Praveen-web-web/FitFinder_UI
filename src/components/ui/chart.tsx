    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item: TooltipEntry, index: number) => {
          ...
          const payloadInner = isRecord(item.payload) ? item.payload : undefined;
          const indicatorColor = color || (payloadInner && (payloadInner.fill as string | undefined)) || (item.color as string | undefined);
...
function ChartLegendContent({
  ...
  // The Legend payload uses a 'Payload' type from DefaultLegendContent types
  const legendPayload = (payload as LegendPayload[] | undefined) ?? undefined;
